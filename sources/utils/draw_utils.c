...
/*
	...
	if (map_y >= game_struct->minimap.map_height
		|| map_x >= game_struct->minimap.map_width)
		return (0);
	if (!game_struct->minimap.map[map_y])
		return (0);
	if (map_x >= (int)ft_strlen(game_struct->minimap.map[map_y]))
		return (0);
...
void	draw_wall_plain_colors(t_struct *game_struct, int t_pix, int b_pix)
{
	...
	if (game_struct->ray->is_active == 1)
	{
		...
		if (game_struct->ray->ray_angle > 0 && game_struct->ray->ray_angle < PI)
			color = BLUE; // South-facing wall (BLUE)
		else
			color = CYAN; // North-facing wall (CYAN)
	}
	else
	{
		// Vertical wall
		if (game_struct->ray->ray_angle > PI/2 && game_struct->ray->ray_angle < 3 * (PI / 2))
			color = YELLOW; // West wall (YELLOW)
		else
			color = MAGENTA; // East wall (MAGENTA)
	}
	// Draw vertical line for this wall slice
	while (t_pix < b_pix)
	{
		draw_game_pixel(game_struct, game_struct->ray->ray_index, t_pix, color);
		t_pix++;
	}
}