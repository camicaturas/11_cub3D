...
/*   Updated: 2025/06/04 13:46:27 by cberneri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_minimap(t_minimap *t_minimap)
{
	...
	t_minimap->nswe ='\0';
//	t_minimap->map = NULL;
	t_minimap->map = calloc(1, sizeof(t_minimap));
}
...
void	init_player(t_struct *game_struct)
{
	...
	game_struct->player.pos.x = floor(game_struct->minimap.player_grid_x * TILE_SIZE + TILE_SIZE / 2); // grid position
	game_struct->player.pos.y = floor(game_struct->minimap.player_grid_y * TILE_SIZE + TILE_SIZE / 2); //  grid position	
}
